...
void shog_generator::compute(anymap_t& data) const
{
	...
	string img_filepath = get<string>(data, "image_filename");
    Mat imgGray;
	...
	// store as csv
	string out_path;
	out_path = img_filepath;
	int id[3]; int count = 0;
	...
	string outDir = "./shog_rand_all";
	string outSubDir = outDir + out_path.substr(id[0], id[1]);
	if (_mkdir(outDir.c_str()) != -1) {
		printf("\nCreate Dir: %s\n", outDir.c_str());
	}
	if (_mkdir(outSubDir.c_str()) != -1) {
		printf("\nCreate Dir: %s\n", outSubDir.c_str());
	}
	const char *p;
	p = out_path.c_str();
	string imgName = p + id[0];
	string outDesPath = outDir + imgName.substr(0, imgName.find_last_of('.')) + ".des";
	string outPosPath = outDir + imgName.substr(0, imgName.find_last_of('.')) + ".pos";

	// For features
	std::ofstream str;
	str.open(outDesPath.c_str());
	if (str.is_open()) {
		imdb::vec_vec_f32_t::iterator it = featuresFiltered.begin();
		for (; it < featuresFiltered.end(); it++) {
			writeVector(*it, str);
			str << std::endl;
		}
	}
	else {
		std::cout << "Failed to Save Features" << std::endl;
	}
	// For positions
	std::ofstream str1;
	str1.open(outPosPath.c_str());
	if (str1.is_open()) {
		imdb::vec_vec_f32_t::iterator it = keypointsNormalizedFiltered.begin();
		for (; it < keypointsNormalizedFiltered.end(); it++) {
			writeVector(*it, str1);
			str1 << std::endl;
		}
	}
	else {
		std::cout << "Failed to Save Positions" << std::endl;
	}
}
...
void shog_generator::extract(const cv::Mat& image, const vec_vec_f32_t& keypoints, vec_vec_f32_t& features, vector<index_t>& emptyFeatures) const
{
    ...
    for (size_t i = 0; i < keypoints.size(); i++)
    {
        ...
        features.push_back(histogram);
		// 单张图片每个采样点的shog特征为 std::vector<float>
		// 单张图片的所有采样点的特征为  std::vector<std::vector<float>>
    }
}

bool shog_registered = Generator::register_generator<shog_generator>("shog");

void shog_generator::writeVector(imdb::vec_f32_t& v, std::ofstream &str) const
{
	imdb::vec_f32_t::iterator it = v.begin();
	str << *it;
	it++;
	for (; it < v.end(); it++) {
		str << ',' << *it;
