...
#define STORE_AS_CSV // store voc centres as csv format
...
void writeVector(imdb::vec_f32_t& v, std::ofstream &str)
{
...
#ifdef STORE_AS_CSV
		// store as csv
		string outVocPath = in_outputfile + "_csv";
		std::ofstream str;
		str.open(outVocPath.c_str());
		if (str.is_open()) {
			imdb::vec_vec_f32_t::iterator it = centers.begin();
			...
			std::cout << "Failed to Save Centres" << std::endl;
		}
#endif
