...
public:
    ...
    command_search()
		...
		, _co_descriptor("descriptor"         , "d", "filename of the descriptors of the query [required]")
    ...
    bool run(const std::vector<std::string>& args)
    {
        ...
        if (!_co_query_image.parse_single<string>(args, in_queryimage) || !_co_filelist.parse_single<string>(args, in_filelist) || !_co_wkdir.parse_single(args, in_wkdir) || !_co_descriptor.parse_single<string>(args, in_descriptor))
        {
            print();
            return false;
        }
		...
		// load the descriptors of the query
		PropertyReaderT<vec_vec_f32_t> reader(in_wkdir + '/' + in_descriptor);
		...
		{
			std::cout << "image_search: progress " << i + 1 << '/' << reader.size() << std::endl;
			...
			if (search_params.get<std::string>("search_type") == "BofSearch")
			{
				...
				const vec_vec_f32_t& samples = boost::any_cast<vec_vec_f32_t>(reader[i]);
